...
from hummingbot.strategy_v2.executors.grid_executor.data_types import GridExecutorConfig, GridLevel, GridLevelStates  # 导入网格执行器数据类型
...
class GridExecutor(ExecutorBase):
    ...
    def _generate_grid_levels(self):
        ...
        if self.config.n_levels is not None and self.config.quote_amount_per_level is not None:
            ...
            # 记录网格创建详情
            self.logger().info(
                f"Created {len(grid_levels)} grid levels with "  # 创建了多少个网格级别
    ...
    def adjust_and_place_open_order(self, level: GridLevel):
        ...
        if order_candidate.amount > 0:  # 如果调整后的数量大于0
            ...
            self.logger().debug(f"Executor ID: {self.config.id} - Placing open order {order_id}")  # 记录调试日志
    ...
    def adjust_and_place_close_order(self, level: GridLevel):
        ...
        if order_candidate.amount > 0:  # 如果数量大于0
            ...
            self.logger().debug(f"Executor ID: {self.config.id} - Placing close order {order_id}")  # 记录调试日志
            ...
            for order in open_orders_placed:  # 遍历这些订单
                ...
                if price:  # 如果价格存在
                    ...
                    if distance_pct > self.config.activation_bounds:  # 如果距离超出激活边界
                        ...
                        self.logger().debug(f"Executor ID: {self.config.id} - Canceling open order {order.order_id}")  # 记录日志
    ...
    def place_close_order_and_cancel_open_orders(self, close_type: CloseType, price: Decimal = Decimal("NaN")):
        ...
        if self.position_size_base >= self.trading_rules.min_order_size:  # 如果当前持仓量大于最小订单大小
            ...
            self.logger().debug(f"Executor ID: {self.config.id} - Placing close order {order_id}")  # 记录日志
